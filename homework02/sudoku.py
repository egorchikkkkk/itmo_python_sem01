...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[T]]:
    ...
    [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    """
    field = [[values[j + n * i] for j in range(n)] for i in range(n)]
    return field
...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    ['2', '8', '.', '.', '.', '5', '.', '7', '9']
    """
    side = int(sqrt(len(grid)))
    # находим, с какой строки и колонки начинать брать блок
    row = (pos[0] // side) * side
    col = (pos[1] // side) * side
    block = []
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    True
    """
    res = set([str(i) for i in range(1, 10)])
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for i in range(len(solution)):
        ...
        if len(numbers) != 9 or "." in numbers:
            return False
        ...
        side = int(sqrt(len(solution)))
        row = (i // 3) * side
        col = (i % 3) * side
        numbers = set(get_block(solution, (row, col)))
        if len(numbers) != 9 or "." in numbers:
            return False
